...
Collapse any value <=7 to boundary of 7 -- otherwise, noise in the 5-7 range can dominate the MAE estimate. Also do MAE_cens which doesn't include values that are censored to 7 in both bgs
...
diffs_bind$MAE_cens <- as.numeric(NA)
...
plot_scatter <- function(site, bg1, bg2, MAE_slope=F, MAE=F, MAE_cens=T, rho=F, cor=F,cutoff=7,phenotype="bind_cens"){
  ...
  plot(x,y, xlim=c(6.5,12),ylim=c(6.5,12),pch=chars,xlab=paste(bg1,phenotype),ylab=paste(bg2,phenotype),col=cols,main=paste("site",site))
...
```{r line_plots_MAE-cens_v_WH1, echo=T, fig.width=12, fig.height=4, fig.align="center", dpi=300,dev="png"}
bg <- "Wuhan_Hu_1"
...
ggplot(data=temp, aes(x=site, y=MAE_cens, group=target, color=target))+
  geom_line(size=0.5)+
