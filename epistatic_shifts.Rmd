...
```{r setup_table}
#define a censored bind measurement that collapses anything <7 to 7 (a lot more noise in the 5-7 range due to concentration range)
dt[,bind_cens := bind]
dt[!is.na(bind) & bind<7,bind_cens:=7]

#compute pi and pi_cens based on bind and bind_cens, as Boltzmann-weighted probabiligy of observing amino-acid a at site r in homolog h
dt[,non_norm_pi:=exp(bind)]
dt[,non_norm_pi_cens:=exp(bind_cens)]

dt[,sum_pi:=sum(non_norm_pi,na.rm=T),by=c("target","position")]
dt[,sum_pi_cens:=sum(non_norm_pi_cens,na.rm=T),by=c("target","position")]

dt[,pi:=non_norm_pi/sum_pi]
dt[,pi_cens:=non_norm_pi_cens/sum_pi_cens]

#define a function for computing J-S divergence between two probability vectors
JS <- function(p,q){
  p_nan <- p[!is.na(p) & !is.na(q)]
  q_nan <- q[!is.na(p) & !is.na(q)]
  n <- 0.5 * (p_nan+q_nan)
  JS <- 0.5 * (sum(p_nan*log(p_nan/n)) + sum(q_nan*log(q_nan/n)))
  return(sqrt(JS))
}
...
diffs_bind$MAE <- as.numeric(NA) #score of mean average error from bind_cens, including values at boundary condition = 7
diffs_bind$MAE_cens <- as.numeric(NA) #score of mean average error from bind_cens, excluding amino acids that are at the boudnary condition = 7 *in both bgs*
diffs_bind$JSD <- as.numeric(NA) #jensen-shannon divergence, from raw bind values (lower limit 5)
diffs_bind$JSD_cens <- as.numeric(NA) #jensen-shannon divergence, from bind_cens values (lower limit 7)
for(i in 1:nrow(diffs_bind)){
  ...
  x_cens <- x[(x>7 | y>7)]
  y_cens <- y[(x>7 | y>7)]
  x_pi <- dt[target==diffs_bind[i,bg_1] & position==diffs_bind[i,site],pi]
  y_pi <- dt[target==diffs_bind[i,bg_2] & position==diffs_bind[i,site],pi]
  x_pi_cens <- dt[target==diffs_bind[i,bg_1] & position==diffs_bind[i,site],pi_cens]
  y_pi_cens <- dt[target==diffs_bind[i,bg_2] & position==diffs_bind[i,site],pi_cens]
  ...
  diffs_bind[i,JSD := JS(x_pi,y_pi)]
  diffs_bind[i,JSD_cens := JS(x_pi_cens,y_pi_cens)]
}
...
plot_scatter <- function(site, bg1, bg2, MAE_slope=F, MAE=F, MAE_cens=F, JSD=T, rho=F, cor=F,cutoff=5,phenotype="bind"){
  ...
  if(MAE_slope==T){
    ...
    x_pi <- dt[target==bg1 & position==site,pi]
    y_pi <- dt[target==bg2 & position==site,pi]
    val <- JS(x_pi,y_pi)
...
TODO: line plots of MAE of focal bg versus other across sites
...
ggplot(data=temp, aes(x=site, y=JSD, group=target, color=target))+
  geom_line(size=0.5)+
...
Same but separate facet per bg comparison
