...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool1A",Kd_ACE2]))` of our pool1A barcodes (`r round(sum(!is.na(dt[library=="pool1A",Kd_ACE2]))/nrow(dt[library=="pool1A",])*100,digits=2)`%), `r sum(!is.na(dt[library=="pool1B",Kd_ACE2]))` of our pool1B barcodes (`r round(sum(!is.na(dt[library=="pool1B",Kd_ACE2]))/nrow(dt[library=="pool1B",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool2A",Kd_ACE2]))` of our pool2A barcodes (`r round(sum(!is.na(dt[library=="pool2A",Kd_ACE2]))/nrow(dt[library=="pool2A",])*100,digits=2)`%). 
...
```{r avgcount, fig.width=12, fig.height=8, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(2,3))
